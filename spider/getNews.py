...
def unfoldList(multList):
    ...
    int_dates = [int('%s%s%s' %
                     (i.year, i.month if i.month >= 10 else '0' +
                      str(i.month), i.day if i.day >= 10 else '0' +
                         str(i.day))) for i in dates_]
    now = datetime.datetime.now()
    int_now = int('%s%s%s' %
                  (now.year, now.month if now.month >= 10 else '0' +
                   str(now.month), now.day if now.day >= 10 else '0' +
                      str(now.day)))
    ...
    for url in urls:
        ...
        try:
            ...
            try:
                ...
                remoteUrls = soup.find_all(
                    "div", class_="title_list_box_130503")[0].find_all('a')
                remoteUrls = remoteUrls[1:]
                remoteUrlsDatas = [(i['href'], i.get_text())
                                   for i in remoteUrls]
                # 调用另一页面方法
